...
RUN --mount=type=cache,target=/root/.cache/go-build \
    ...
    go install \
        ...
        -ldflags " \
            ...
            -X github.com/cosmos/cosmos-sdk/version.Name='merlind' \
...
FROM alpine:${ALPINE_VERSION} as merlin-chain

COPY --from=builder-stage-2 /go/bin/merlind /usr/local/bin/merlind

RUN addgroup -g 1000 merlin-chain && \
    adduser -u 1000 -G merlin-chain -D -h /app merlin-chain

WORKDIR /app

CMD ["merlind", "--home", "/app", "start"]