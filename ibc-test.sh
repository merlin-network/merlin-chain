...
fanfuryd tx intertx register --from demowallet2 --connection-id connection-0 --chain-id test-2 --home ./data/test-2 --keyring-backend test -y --broadcast-mode=block
...
export DEMOWALLET_2=$(fanfuryd keys show -a demowallet2 --home ./data/test-2 --keyring-backend test)
fanfuryd query intertx interchainaccounts connection-0 $DEMOWALLET_2
...
# fanfuryd q ibc channel channels;

# icad keys list --keyring-backend=test --home=./data/test-1

# icad tx ibc-transfer transfer transfer channel-1 fury1t7cyvydpp4lklprksnrjy2y3xzv3q2l0p27xyv 1000000stake --chain-id=test-1 --from=demowallet1 --keyring-backend=test -y --broadcast-mode=block --home=./data/test-1 --node=http://localhost:16657

# fanfuryd query bank balances fury1t7cyvydpp4lklprksnrjy2y3xzv3q2l0p27xyv 
