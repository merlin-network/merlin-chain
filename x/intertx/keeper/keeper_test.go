...
import (
	...
	icaapp "github.com/merlin-network/merlin-chain/app"
...
var (
	...
	TestOwnerAddress = "merlin1665x2fj8xyez0vqxs5pjhc6e7ktmmrx9laeuaj"
...
func SetupICATestingApp() (ibctesting.TestingApp, map[string]json.RawMessage) {
	...
	app := icaapp.NewMerlinApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, icaapp.DefaultNodeHome, 5, encCdc, simapp.EmptyAppOptions{})
...
func (suite *KeeperTestSuite) GetICAApp(chain *ibctesting.TestChain) *icaapp.MerlinApp {
	app, ok := chain.App.(*icaapp.MerlinApp)
	if !ok {
		panic("not merlin app")
	}
